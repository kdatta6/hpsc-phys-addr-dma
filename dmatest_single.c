...
static int dmatest_with_virt_addr(u8 *virt_addr,
...
static int dmatest_func(void *data)
{
	...
	printk("In dmatest_func(), buf_size = %u, align = %u", buf_size, align);

	src_phys_addr = 0x83600000;
	src_virt_addr = memremap(src_phys_addr, buf_size, MEMREMAP_WB);
	printk("Before call to dmatest_with_virt_addr(), src_phys_addr = %lx, src_virt_addr = %p\n", src_phys_addr, src_virt_addr);
	dmatest_with_virt_addr(src_virt_addr, src, buf_size, align);

	dst_phys_addr = 0x83601000;
	dst_virt_addr = memremap(dst_phys_addr, buf_size, MEMREMAP_WB);
	printk("Before call to dmatest_with_virt_addr(), dst_phys_addr = %lx, dst_virt_addr = %p\n", dst_phys_addr, dst_virt_addr);
	dmatest_with_virt_addr(dst_virt_addr, dst, buf_size, align);

	/*
	printk("Before calls to dmatest_alloc_test_data()\n");
	...
	*/
	...
	while (!kthread_should_stop()
		...
		src_virt_addr = src->aligned[0];
		printk("\nIn dmatest.c dmatest_func() BEFORE dmatest_init_srcs, src_virt_addr = %p\n", (void *)src_virt_addr);
		src_val = readl(src_virt_addr+src->off);
		printk("In dmatest.c BEFORE dmatest_init_srcs, %u (src->off) B after src_virt_addr, val = 0x%x\n", src->off, src_val);
		dst_virt_addr = dst->aligned[0];
		printk("\nIn dmatest.c dmatest_func() BEFORE dmatest_init_dsts, dst_virt_addr = %p\n", (void *)dst_virt_addr);
		dst_val = readl(dst_virt_addr+dst->off);
		printk("In dmatest.c BEFORE dmatest_init_dsts, %u (dst->off) B after dst_virt_addr, val = 0x%x\n", dst->off, dst_val);
		...
		printk("\nIn dmatest.c dmatest_func() AFTER dmatest_init_srcs, src_virt_addr = %p\n", (void *)src_virt_addr);
		src_val = readl(src_virt_addr);
		printk("In dmatest.c AFTER dmatest_init_srcs, 0 B after src_virt_addr, val = 0x%x\n", src_val);
		src_val = readl(src_virt_addr+src->off);
		printk("In dmatest.c AFTER dmatest_init_srcs, %u (src->off) B after src_virt_addr, val = 0x%x\n", src->off, src_val);
		printk("\nIn dmatest.c dmatest_func() AFTER dmatest_init_dsts, dst_virt_addr = %p\n", (void *)dst_virt_addr);
		dst_val = readl(dst_virt_addr);
		printk("In dmatest.c AFTER dmatest_init_dsts, 0 B after dst_virt_addr, val = 0x%x\n", dst_val);
		dst_val = readl(dst_virt_addr+dst->off);
		printk("In dmatest.c AFTER dmatest_init_dsts, %u (dst->off) B after dst_virt_addr, val = 0x%x\n", dst->off, dst_val);
		...
		for (i = 0; i < src->cnt; i++) {
			...
			*/
			srcs[i] = dma_map_single(dev->dev, (buf + src->off), um->len, DMA_TO_DEVICE);
			printk("srcs[i] = %p\n", (void *)(srcs[i]));
		...
		for (i = 0; i < dst->cnt; i++) {
			...
			*/
			dsts[i] = dma_map_single(dev->dev, buf, um->len, DMA_BIDIRECTIONAL);
			printk("dsts[i] = %p\n", (void *)(dsts[i]));
			ret = dma_mapping_error(dev->dev, dsts[i]);
			if (ret) {
		...
		printk("\nIn dmatest_func(), src->off = %u\n", src->off); 
		printk("In dmatest_func(), len = %u\n", len);
		printk("In dmatest_func(), buf_size = %u\n", buf_size);
		printk("In dmatest_func(), src->aligned = %p\n", (void *)(src->aligned));

		src_virt_addr = src->aligned[0];
		printk("In dmatest.c dmatest_func() AFTER DMA, src_virt_addr = %p\n", (void *)src_virt_addr);
		src_val = readl(src_virt_addr);
		printk("In dmatest.c AFTER DMA, 0 B after src_virt_addr, val = 0x%x\n", src_val);
		src_val = readl(src_virt_addr+src->off);
		printk("In dmatest.c AFTER DMA, %u (src->off) B after src_virt_addr, val = 0x%x\n", src->off, src_val);
		...
		printk("\nIn dmatest_func(), dst->off = %u\n", dst->off); 
		printk("In dmatest_func(), len = %u\n", len);
		printk("In dmatest_func(), buf_size = %u\n", buf_size);
		printk("In dmatest_func(), dst->aligned = %p\n", (void *)(dst->aligned));

		dst_virt_addr = dst->aligned[0];
		printk("In dmatest.c dmatest_func() AFTER DMA, dst_virt_addr = %p\n", (void *)dst_virt_addr);
		dst_val = readl(dst_virt_addr);
		printk("In dmatest.c AFTER DMA, 0 B after dst_virt_addr, val = 0x%x\n", dst_val);
		dst_val = readl(dst_virt_addr+dst->off);
		printk("In dmatest.c AFTER DMA, %u (dst->off) B after dst_virt_addr, val = 0x%x\n", dst->off, dst_val);
		...
		memunmap((void *)src_phys_addr);
		memunmap((void *)dst_phys_addr);
...
static void add_threaded_test(struct dmatest_info *info)
{
	...
	printk("Starting add_threaded_test()...\n");
	...
	printk("In add_threaded_test() before DMA_MEMCPY request_channels()...\n");
	request_channels(info, DMA_MEMCPY);
	printk("In add_threaded_test() before DMA_MEMSET request_channels()...\n");
	request_channels(info, DMA_MEMSET);
	printk("In add_threaded_test() before DMA_XOR request_channels()...\n");
	request_channels(info, DMA_XOR);
	printk("In add_threaded_test() before DMA_PQ request_channels()...\n");
	request_channels(info, DMA_PQ);
	printk("Ending add_threaded_test()...\n");
}
...
static void start_threaded_tests(struct dmatest_info *info)
{
	...
	printk("In start_threaded_tests()\n");
	run_pending_tests(info);
	printk("Ending start_threaded_tests()\n");
}
...
static int dmatest_run_set(const char *val, const struct kernel_param *kp)
{
	...
	printk("Starting dmatest_run_set()\n");
	mutex_lock(&info->lock);
...
static int dmatest_chan_set(const char *val, const struct kernel_param *kp)
{
	...
	printk("Starting dmatest_chan_set()...");
	mutex_lock(&info->lock);
	...
	mutex_unlock(&info->lock);
	printk("Ending dmatest_chan_set()...");
...
static int __init dmatest_init(void)
{
	...
	printk("In dmatest_init(), before check of dmatest_run condition\n");
	if (dmatest_run) {
		mutex_lock(&info->lock);
		add_threaded_test(info);
		run_pending_tests(info);
	...
	printk("In dmatest_init(), after check of dmatest_run condition\n");
